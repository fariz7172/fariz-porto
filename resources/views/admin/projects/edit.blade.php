                        ...
                        <div class="grid gap-6">
                            ...
                            <div>
                                ...
                                <textarea name="full_description" id="full_description" rows="6"
                                    class="w-full rounded-lg border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500"
                                    placeholder="Deskripsi detail project untuk modal popup...">{{ old('full_description', $project->full_description) }}</textarea>
                            ...
                            <!-- Project URL -->
                            <div>
                                <label for="project_url" class="block text-sm font-medium text-gray-700 mb-2">Link Project (URL)</label>
                                <input type="url" name="project_url" id="project_url" 
                                    value="{{ old('project_url', $project->project_url) }}"
                                    class="w-full rounded-lg border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500"
                                    placeholder="https://example.com atau https://github.com/username/repo">
                                <p class="mt-1 text-sm text-gray-500">Link demo atau repository project (akan tampil sebagai tombol di modal)</p>
                                @error('project_url')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="grid md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <select name="color" id="color" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-green-500 focus:ring-green-500" required>
                                        <option value="from-blue-500 to-indigo-600" {{ old('color', $project->color) == 'from-blue-500 to-indigo-600' ? 'selected' : '' }}>Blue → Indigo</option>
                                        <option value="from-emerald-500 to-teal-600" {{ old('color', $project->color) == 'from-emerald-500 to-teal-600' ? 'selected' : '' }}>Emerald → Teal</option>
                                        <option value="from-orange-500 to-red-600" {{ old('color', $project->color) == 'from-orange-500 to-red-600' ? 'selected' : '' }}>Orange → Red</option>
                                        <option value="from-purple-500 to-pink-600" {{ old('color', $project->color) == 'from-purple-500 to-pink-600' ? 'selected' : '' }}>Purple → Pink</option>
                                        <option value="from-cyan-500 to-blue-600" {{ old('color', $project->color) == 'from-cyan-500 to-blue-600' ? 'selected' : '' }}>Cyan → Blue</option>
                                        <option value="from-amber-500 to-orange-600" {{ old('color', $project->color) == 'from-amber-500 to-orange-600' ? 'selected' : '' }}>Amber → Orange</option>
                                        <option value="from-rose-500 to-pink-600" {{ old('color', $project->color) == 'from-rose-500 to-pink-600' ? 'selected' : '' }}>Rose → Pink</option>
                                        <option value="from-green-500 to-emerald-600" {{ old('color', $project->color) == 'from-green-500 to-emerald-600' ? 'selected' : '' }}>Green → Emerald</option>
                                    </select>
                                    @error('color')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
